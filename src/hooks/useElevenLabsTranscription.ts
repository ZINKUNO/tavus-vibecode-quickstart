...
import { ElevenLabsTranscriber, ElevenLabsTranscriptionResult, ElevenLabsConfig } from '../lib/elevenlabs';
...
export const useElevenLabsTranscription = (options: UseElevenLabsTranscriptionOptions = {}) => {
  const [transcriptions, setTranscriptions] = useState<ElevenLabsTranscriptionResult[]>([]);
  const [isTranscribing, setIsTranscribing] = useState(false);
  const [transcriptionError, setTranscriptionError] = useState<string | null>(null);
  const [isInitializing, setIsInitializing] = useState(false);
  
  const transcriberRef = useRef<ElevenLabsTranscriber | null>(null);
  const userStreamRef = useRef<MediaStream | null>(null);

  const handleTranscription = useCallback((result: ElevenLabsTranscriptionResult) => {
  ...
  const initializeTranscriber = useCallback(() => {
    if (!transcriberRef.current) {
      transcriberRef.current = new ElevenLabsTranscriber(
        handleTranscription,
        handleError,
        options.config
      );
  ...
  const updateConfig = useCallback((config: Partial<ElevenLabsConfig>) => {
    if (transcriberRef.current) {
  ...
  const exportTranscriptions = useCallback((format: 'txt' | 'json' | 'srt' = 'txt') => {
    ...
    let content = '';
    let filename = `elevenlabs-transcript-${new Date().toISOString().split('T')[0]}`;
